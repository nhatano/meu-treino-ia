...
# LINK DO SEU FORMULÁRIO (Substitua após criar o form)
# Exemplo: https://docs.google.com/forms/d/e/1FAIpQLS.../formResponse
FORM_URL = "SUA_URL_AQUI/formResponse"
...
if user_email in [MEU_EMAIL, SAYRA_EMAIL]:
    ...
    st.markdown(f"### Bem-vindo de volta, **{u_name}**!")
    
    tab1, tab2 = st.tabs(["TREINOS", "DICAS"])
            ...
            with st.container():
                ...
                with cd:
                    for i, meta in enumerate(item['sets']):
                        ...
                        cols[0].text_input("Alvo", value=meta, key=f"t_{item['ex']}_{i}")
                        cols[1].number_input("Peso (kg)", key=f"w_{item['ex']}_{i}", min_value=0)
                        cols[2].selectbox("RPE", list(range(1,11)), index=7, key=f"r_{item['ex']}_{i}")
                
                if st.button(f"Salvar {item['ex']}", key=f"btn_{item['ex']}"):
                    # No modo simples, vamos apenas simular o sucesso para não travar seu treino hoje!
                    st.success(f"Excelente, {u_name}! Série de {item['ex']} anotada mentalmente.")
                    st.balloons()
