...
MEU_EMAIL = "seu_email@gmail.com" # <--- SUBSTITUA PELO SEU E-MAIL REAL
SAYRA_EMAIL = "sayradan@gmail.com"

# --- ESTRUTURA COMPLETA DE TREINOS (TODOS OS EXERCÍCIOS) ---
TREINOS_COMPLETOS = {
    ...
    "SÁBADO: TREINO 5 - PUSH B (OMBROS 3D & PEITO)": [
        ...
        {"ex": "Crucifixo Inverso Máquina", "sets": ["33kg", "40kg", "40kg (Falha)"], "bio": "Deltoide posterior.", "vid": "https://youtu.be/Xp0NfT_SjG0"},
...
if email_login in [MEU_EMAIL, SAYRA_EMAIL]:
    ...
    if modo == "👊 Treinar Agora":
            ...
            with st.expander(f"🔥 {item['ex']}", expanded=False):
                ...
                with col_d:
                    for i, meta in enumerate(item['sets']):
                        ...
                        c1.write(f"**Série {i+1}**")
                        c1.markdown(f"<div class='meta-text'>Alvo: {meta if email_login == MEU_EMAIL else 'A definir'}</div>", unsafe_allow_html=True)
                        c2.number_input(f"Carga", key=f"c_{item['ex']}_{i}_{escolha}", min_value=0)
        ...
        duvida = st.text_area("Ex: 'Substituição para o Hack?' ou 'Dicas de progressão'.")
        if st.button("Consultar IA"):
            with st.spinner("O Coach está pensando..."):
                resp = model.generate_content(f"Usuário {u_nome} (Avançado). Dúvida: {duvida}")
                st.info(resp.text)
else:
    st.info("👋 Por favor, digite seu e-mail e clique em 'Carregar Treino'.")
